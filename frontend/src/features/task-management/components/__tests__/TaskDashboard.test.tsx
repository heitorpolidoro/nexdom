...
describe("TaskDashboard", () => {
  beforeEach(() => {
    ...
    vi.mocked(useCreateTask).mockReturnValue({ mutate: vi.fn(), isPending: false } as any);
    vi.mocked(useUpdateTask).mockReturnValue({ mutate: vi.fn(), isPending: false } as any);
    vi.mocked(useDeleteTask).mockReturnValue({ mutate: vi.fn(), isPending: false } as any);
    vi.mocked(useTaskHistory).mockReturnValue({ data: [], isLoading: false } as any);
    vi.mocked(useUsersHook.useUsers).mockReturnValue({ data: [], isLoading: false } as any);
  });
  ...
  it("renders the dashboard header and filters", () => {
    ...
    expect(screen.getByDisplayValue("Todas as prioridades")).toBeInTheDocument();
  });
  ...
  it("handles successful task creation/update and closes overlay", () => {
    ...
    vi.mocked(useCreateTask).mockReturnValue({ mutate: mockMutate, isPending: false } as any);
  ...
  it("closes overlay when clicking on background", () => {
    ...
    const modalContent = screen.getByRole("heading", { name: "Nova Tarefa" }).closest('div');
  ...
  it("resets other modes when handleTaskClick is called", () => {
    ...
    fireEvent.click(screen.getByText("Task 1"));
    expect(screen.getByText("Fechar")).toBeInTheDocument(); // Details view
    expect(screen.queryByRole("heading", { name: "Nova Tarefa" })).not.toBeInTheDocument(); // Create form should be gone
  });
  ...
  it("closes overlay after successful update in edit mode", () => {
    ...
    vi.mocked(useUpdateTask).mockReturnValue({ mutate: mockUpdateMutate, isPending: false } as any);
  ...
  it("handles loading and error states from useTasks", () => {
    ...
    render(<TaskDashboard />);
    expect(screen.getByText(/Erro ao carregar tarefas: Fetch error/i)).toBeInTheDocument();
  });
});
